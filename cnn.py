...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):  
        ...
        self.layering_in_cnn.append(inputs)
        self.model = keras.models.Model(inputs=self.layering_in_cnn[0], outputs= self.layering_in_cnn[-1])
        """
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        dense_layer = keras.layers.Dense(num_nodes, activation = activation, name = name, trainable = trainable)(self.model.layers[-1].output)
        self.layering_in_cnn.append(dense_layer)
        self.model = keras.models.Model(inputs=self.model.layers[0].output, outputs= self.layering_in_cnn[-1])
        """
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1, activation="Relu",name="",trainable=True):
        conv_2d_layer = keras.layers.Conv2D(filters=num_of_filters, kernel_size = kernel_size, padding=padding, activation=activation, strides = strides, trainable = trainable, name = name)(self.model.layers[-1].output)
        self.layering_in_cnn.append(conv_2d_layer)
        self.model = keras.models.Model(inputs=self.model.layers[0].output, outputs= self.layering_in_cnn[-1])
        """
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        pool_max = keras.layers.MaxPooling2D(pool_size = pool_size, padding = padding, strides = strides, name = name)(self.model.layers[-1].output)
        self.layering_in_cnn.append(pool_max)
        self.model = keras.models.Model(inputs=self.model.layers[0].output, outputs= self.layering_in_cnn[-1])
        """
    ...
    def append_flatten_layer(self,name=""):
        flattened_layer = tf.keras.layers.Flatten(name=name)(self.model.layers[-1].output)
        self.layering_in_cnn.append(flattened_layer)
        self.model = keras.models.Model(inputs=self.model.layers[0].output, outputs= self.layering_in_cnn[-1])
        """
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if( type(layer_numbers) != list):      
            list_layer_number.append(layer_numbers)     
        if( type(layer_names) != list):   
            list_layer_name.append(layer_names)      
        list_layer_number = list(layer_numbers)
        list_layer_name = list(layer_names)
        if(len(list_layer_number) != 0):
            for i in range(len(list_layer_number)):           
                self.model.layers[list_layer_number[i]].trainable = trainable_flag     
        elif(len(list_layer_name) != 0):    
            for i in range(len(list_layer_name)):      
                self.model.get_layer(list_layer_name[i]).trainable = trainable_flag
        """
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):   
        if(layer_number != None):
            try:
                self.model.layers[layer_number].set_weights([weights,self.model.layers[layer_number].get_weights()[1]])
            except:
                return None              
        elif(layer_name != ""):           
            try:
                    self.model.get_layer(name=layer_name).set_weights([weights,self.model.get_layer(name=layer_name).get_weights()[1]])
            except:
                    return None
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):       
        if(layer_number != None):
            try:
                self.model.layers[layer_number].set_weights([self.model.layers[layer_number].get_weights()[0], biases])
            except:
                return None
        elif(layer_name != ""):   
            try:
                self.model.get_layer(name=layer_name).set_weights([self.model.get_layer(name=layer_name).get_weights()[0], biases])
            except:
                    return None
        """
    ...
    def remove_last_layer(self):
        last_layer_to_remove = self.layering_in_cnn[-1]    
        self.layering_in_cnn = self.layering_in_cnn[:len(self.layering_in_cnn)-1]      
        self.model = keras.models.Model(inputs=self.layering_in_cnn[0], outputs= self.layering_in_cnn[-1])     
        return last_layer_to_remove
        """
    ...
    def load_a_model(self,model_name="",model_file_name=""):    
        ...
        elif(model_name == "" and model_file_name != ""):
                self.model = keras.models.load_model(model_file_name)
        self.layering_in_cnn = self.model.layers
        return self.model
    ...
    def evaluate(self,X,y):
        evaluate_the_model = self.model.evaluate(x=X, y=y)
        return evaluate_the_model
        """
    ...
    def train(self, X_train, y_train, batch_size, num_epochs):
        self.model.compile(optimizer=self.optimizer,loss=self.loss,metrics=[self.metric],verbose=0)
        history = self.model.fit(X_train, y_train, batch_size=batch_size, epochs=num_epochs,verbose=0)
